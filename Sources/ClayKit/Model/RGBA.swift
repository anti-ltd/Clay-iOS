/**
 `RGBA`: a `Codable`, `Sendable` color value used throughout the theme model.
 Bridges to SwiftUI `Color` and UIKit `UIColor` without importing either in the
 model layer. Ported from ClingKit/ClinkKit so the family shares one color idiom.
 */
import SwiftUI
#if canImport(UIKit)
import UIKit
...
/// Codable, value-type color so a theme can round-trip through the App Group
/// as JSON. SwiftUI's `Color` is not reliably Codable across OS versions, so
/// we store raw sRGB components and bridge to `Color` on demand.
public struct RGBA: Codable, Equatable, Sendable, Hashable {
    ...
    /// theme editor's `ColorPicker` to capture a chosen colour.
